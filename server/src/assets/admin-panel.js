...
async function showDashboard() {
  ...
  await Promise.all([loadCheats(), loadUsers(), loadSessions(), loadResets(), loadDashboardStats()]);
}
...
async function loadDashboardStats() {
  try {
    ...
    // Update stat cards
    document.getElementById('statCheats').textContent = s.totalCheats;
    ...
    document.getElementById('statResets').textContent = s.pendingResets;

    const chartColors = {
      green: '#00e68a', blue: '#3b82f6', gold: '#ffd700',
      purple: '#a855f7', red: '#ff4757', cyan: '#06b6d4'
    };
    const chartDefaults = {
      color: '#8892a4', borderColor: 'rgba(255,255,255,0.06)',
      font: { family: 'Inter' }
    };

    // User Growth (line)
    renderChart('chartUserGrowth', 'line', {
      labels: s.usersByDay.map(r => r.date ? new Date(r.date).toLocaleDateString('en', { month: 'short', day: 'numeric' }) : ''),
      datasets: [{ label: 'New Users', data: s.usersByDay.map(r => r.count), borderColor: chartColors.green, backgroundColor: 'rgba(0,230,138,0.1)', fill: true, tension: 0.4, pointRadius: 3 }]
    });

    // Plan Distribution (doughnut)
    renderChart('chartPlans', 'doughnut', {
      labels: s.planDistribution.map(r => (r.plan || 'free').toUpperCase()),
      datasets: [{ data: s.planDistribution.map(r => r.count), backgroundColor: [chartColors.gold, chartColors.purple, chartColors.blue, chartColors.green, chartColors.cyan] }]
    }, { plugins: { legend: { position: 'right', labels: { color: '#8892a4', padding: 12 } } } });

    // Top Cheats (bar)
    renderChart('chartTopCheats', 'bar', {
      labels: s.topCheats.map(r => r.name),
      datasets: [{ label: 'Licenses', data: s.topCheats.map(r => r.licenses), backgroundColor: [chartColors.green, chartColors.blue, chartColors.gold, chartColors.purple, chartColors.cyan], borderRadius: 6 }]
    }, { indexAxis: 'y' });

    // Sessions (line)
    renderChart('chartSessions', 'line', {
      labels: s.sessionsByDay.map(r => r.date ? new Date(r.date).toLocaleDateString('en', { month: 'short', day: 'numeric' }) : ''),
      datasets: [{ label: 'Sessions', data: s.sessionsByDay.map(r => r.count), borderColor: chartColors.blue, backgroundColor: 'rgba(59,130,246,0.1)', fill: true, tension: 0.4, pointRadius: 3 }]
    });
...
function renderChart(canvasId, type, data, extraOpts = {}) {
  ...
  chartInstances[canvasId] = new Chart(ctx, {
    ...
    options: {
      ...
      plugins: { legend: { labels: { color: '#8892a4' } }, ...extraOpts.plugins },
